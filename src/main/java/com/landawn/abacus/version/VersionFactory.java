    ...
    public static <K> Version<K> createVersion(String provider) {
        ...
        if (MEMCACHED.equalsIgnoreCase(className)) {
            if (parameters.length == 1) {
                ...
                return new DistributedVersion<>(new SpyMemcached<Long>(url, DEFAULT_TIMEOUT), parameters[1], Numbers.toLong(parameters[2]));
            } else if (parameters.length == 4) {
                return new DistributedVersion<>(new SpyMemcached<Long>(url, Numbers.toLong(parameters[3])), parameters[1], Numbers.toLong(parameters[2]));
            } else {
            ...
            if (parameters.length == 1) {
                ...
                return new DistributedVersion<>(new JRedis<Long>(url, DEFAULT_TIMEOUT), parameters[1], Numbers.toLong(parameters[2]));
            } else if (parameters.length == 4) {
                return new DistributedVersion<>(new JRedis<Long>(url, Numbers.toLong(parameters[3])), parameters[1], Numbers.toLong(parameters[2]));
            } else {
                throw new IllegalArgumentException("Unsupported parameters: " + StringUtil.join(parameters));
            }
        } else {
