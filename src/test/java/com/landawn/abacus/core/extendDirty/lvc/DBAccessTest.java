...
public class DBAccessTest extends AbstractEntityManager1Test {
    ...
    public void test_query() {
        ...
        dbAccounts = dbAccess.list(Account.__, Account._PNL, CF.eq(Account.ID, dbAccount.getId()), options);
        assertEquals(1, dbAccounts.size());

        dbAccess.delete(Account.__, CF.ge(Account.ID, dbAccount.getId()), null);
    }
    ...
    public void testCRUD() {
        ...
        List<Account> dbAccounts = dbAccess.list(Account.__, Account._PNL, CF.eq(Account.ID, dbAccount.getId()));
        assertEquals(1, dbAccounts.size());
        ...
        dbAccounts = dbAccess.list(Account.__, Account._PNL, CF.eq(Account.ID, dbAccount.getId()), null);
    ...
    @Test
    public void testGet_AllSelectPropNames() {
        Map<String, Object> props = createAccountProps();
        EntityId entityId = dbAccess.add(Account.__, props, null);

        Account dbAccount = dbAccess.gett(entityId, Account._PNL, null);
        assertTrue(dbAccount.getId() > 0);
        assertEquals(props.get(Account.FIRST_NAME), dbAccount.getFirstName());

        List<Account> dbAccounts = dbAccess.list(Account.__, Account._PNL, null, null);
        assertTrue(dbAccounts.get(0).getId() > 0);
        assertEquals(props.get(Account.FIRST_NAME), dbAccounts.get(0).getFirstName());
    }
    ...
    public void testGet_EmptySelectPropNames() {
        ...
        assertTrue(dbAccount.getId() > 0);
        assertEquals(null, dbAccount.getFirstName());

        List<Account> dbAccounts = dbAccess.list(Account.__, selectPropNames, null, null);
        assertTrue(dbAccounts.get(0).getId() > 0);
        assertEquals(null, dbAccounts.get(0).getFirstName());
    }
    ...
    public void testAdd_byPropsList_2() {
        Map<String, Object> props1 = createAccountProps();
        props1.remove(Account.FIRST_NAME);

        Map<String, Object> props2 = createAccountProps();
        props2.remove(Account.LAST_NAME);

        List<Map<String, Object>> propsList = N.asList(props1, props2);
    ...
    public void testUpdate_1() {
        ...
        // ----------------------------------------------
        try {
            dbAccess.update(Account.__, N.asProps(), null, null);
            fail("should throw IllegalArgumentException");
        } catch (IllegalArgumentException e) {
        }
    ...
    public void testGetResultByHandle() {
        assertFalse(null instanceof Double);
        ...
        assertEquals(4, dataSet.columnNameList().size());

        dataSet = dbAccess.getResultByHandle(resultHandle.value(), N.asList(Account.CONTACT), N.asProps(Options.Query.OFFSET, 3, Options.Query.COUNT, 3));
    ...
    public void testGetEntityDefinitionFactory() {
        ...
        assertEquals(11, dbAccess.getEntityDefinitionFactory().getEntityNameList().size());
    }
    ...
    public void testQuery() {
        ...
        Collection<String> selectPropNames = N.asList(Account.FIRST_NAME, Account.CONTACT, AccountContact.ADDRESS);
        dbAccess.query(Account.__, selectPropNames, null).println();

        selectPropNames = N.asList("id", "firstName");
        dbAccess.query(Account.__, selectPropNames, null).println();
    }
